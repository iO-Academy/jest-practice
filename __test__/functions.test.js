 ...
 */
test('Age is 30', () => {
    expect(functions.calculateAge('1992-01-01')).toBe(30);
})

test('Age is 0 for current year', () => {
    expect(functions.calculateAge('2022-01-01')).toBe(0);
})

test('Returns false for future year', () => {
    expect(functions.calculateAge('2050-01-01')).toBe(false);
})

// Failure test
test('Invalid date returns false', () => {
    expect(functions.calculateAge('2022-13-01')).toBe(false);
})

// Malformed test
test('Incorrect data type returns false', () => {
    expect(functions.calculateAge({})).toBe(false);
})
...
test('Below minimum threshold returns 0 tax', () => {
    expect(functions.calculateTax(12499)).toBe(0);
})

test('First tax bracket applies 5% tax', () => {
    expect(functions.calculateTax(12600)).toBe(5);
})

test('Second tax bracket applies 10% tax', () => {
    expect(functions.calculateTax(30100)).toBe(885);
})

test('Third tax bracket applies 20% tax', () => {
    expect(functions.calculateTax(50100)).toBe(2895);
})

// Failure test
test('Negative salary returns 0', () => {
    expect(functions.calculateTax(-100)).toBe(0);
})

// Malformed test
test('Incorrect data type returns 0', () => {
    expect(functions.calculateTax([])).toBe(0);
})
...
test('Greeting returned without weather forecast when passing false param', () => {
    const greeting = functions.greet('Mike', false)
    expect(greeting).toMatch(/Hi Mike./);
...
test('Anonymous greeting returned with incorrect name param data type', () => {
    const greeting = functions.greet([])
